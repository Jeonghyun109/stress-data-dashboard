...
const TreemapTemplate: React.FC<{
  ...
  return <>
    ...
    }{josa.pick(top3.at(-1)?.name ?? '', '이였습니다/였습니다')}.<br />
    ...
    상황 데이터에서는 <strong style={{ color: '#33A1FD' }}>{topData.context?.join(', ')}</strong>{josa.pick(topData.context?.at(-1) ?? '', '이/가')} 가장 관련이 있었습니다.
    일일 상황 데이터에서는 <strong style={{ color: '#1c1e7a' }}>{topData.daily_context?.join(', ')}</strong>{josa.pick(topData.daily_context?.at(-1) ?? '', '이/가')} 가장 연관이 있었습니다. <br /><br />
