...
const levelToClass = (type: Stress, lvl: number) => {
  ...
  return STRESS_CLASSES[type][Math.min(4, Math.abs(lvl))] ?? 'bg-white';
};

const levelToHex = (type: Stress, lvl: number) => {
  if (Number.isNaN(lvl)) return '#ffffff';
  if (lvl > 0) return '#fecaca';
  return STRESS_HEX[type][Math.min(4, Math.abs(lvl))] ?? '#ffffff';
};
...
const Calendar: React.FC<CalendarProps> = ({ pid, selectedDate, setSelectedDate }) => {
  ...
  const { getForDate } = useStressDiffData('/data/diff_full.csv', pid);
  ...
  return (
    <div className="w-[338px] mx-auto font-sans py-6 pl-0 pr-6">
      <CalendarHeader
        showPsych={showPsych}
        setShowPsych={setShowPsych}
        showPhys={showPhys}
        setShowPhys={setShowPhys}
        psychColor="#2dd4bf"
        physColor="#a3e635"
      />
      <MonthNavigation
      ...
      />
      <WeekLabels labels={WEEK_DAYS} className="mt-6" />
